...
client.on('messageCreate', async (message) => {
    ...
    const prefix = await client.db.get(`guild_prefix_${message.guild.id}`) || client.config.Prefix;
    ...
    if (command) {
        ...
        if (command.owner, command.owner == true) {
            if (client.config.Users?.OWNERS) {
                const allowedUsers = []; // New Array.

                client.config.Users.OWNERS.forEach(user => {
                    const fetchedUser = message.guild.members.cache.get(user);
                    if (!fetchedUser) return allowedUsers.push('*Unknown User#0000*');
                    allowedUsers.push(`${fetchedUser.user.tag}`);
                })

                if (!client.config.Users.OWNERS.some(ID => message.member.id.includes(ID))) return message.reply({
                    embeds: [
                        new EmbedBuilder()
